...
export async function hasFramesForDate(date: Date): Promise<boolean> {
	try {
		...
		// Keep this check aligned with the calendar day list: an audio-only day
		// is still a real timeline day and must not be skipped by navigation.
		// Use SELECT 1 ... LIMIT 1 instead of COUNT(*) so either indexed branch
		// can short-circuit after its first match.
		const query = `
...
export async function findNearestDateWithFrames(
	...
	try {
		...
		// Screen and audio timestamps are both timeline positions. Query both so
		// the date picker and arrows can enter days where screenshots were paused
		// but audio capture continued.
		// For backward: use the most recent timestamp (ORDER BY DESC).
		// For forward: use the earliest timestamp (ORDER BY ASC).

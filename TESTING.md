...
- [ ] **Reconnect mid date-swap** — Kill/restart the engine while a past-day swap is pending. Verify reconnect re-fetches with prior-day context and the strip does not land as a half strip.
- [ ] **Today button mid-swap** — Start a past-day navigation, then hit Today before it completes. Verify the swap cancels and today loads without a stuck date-swap skeleton overlay. Late batches from the cancelled past-day fetch must not merge into the strip.
...
- [ ] **Same-day calendar re-pick** — Already on a day with frames loaded; pick that day again from the calendar. Verify the playhead lands near the day's start (oldest end), not the newest same-day frame.
...
- [ ] **Strip skeleton icon chips** — Day-change and edge-prefetch skeletons use solid opacity-pulse pills with blank ~14px icon chips cycling soft / squircle / sharp (no fake app glyphs, no glass/shimmer).
...
- [ ] **Edge prefetch skeleton** — Scroll toward the end of the loaded day (either direction). Prefetch should arm ~2 viewports before the loaded frame-array edge (segmented skeleton continues the strip), skip empty gap days, and not abort solely because a nav multi-day range marked an empty day as "fetched". Skeleton clears atomically with the frame flush (no scroll bump). On today, after an older-day edge prefetch completes, new live frames should keep arriving.

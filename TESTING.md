...
- [ ] **Edge prefetch skeleton** — Scroll toward the end of the loaded day (either direction). Prefetch should arm ~2 viewports before the loaded frame-array edge (segmented skeleton continues the strip), skip empty gap days, and still probe a gap day whose midnight was only covered by a prior multi-day nav range. Skeleton clears atomically with the frame flush (no scroll bump). On today, after an older-day edge prefetch completes, new live frames should keep arriving.

...
- [ ] **Calendar jump to historical day** — Pick a day weeks/months back from the calendar. Verify the timeline switches to that day (no snap-back to today), even when the fetch takes 30s+; the date pill shows a spinner while a segmented strip skeleton overlays the strip (strip stays mounted underneath, playhead chip hidden), then the day swaps in and the skeleton clears.
...
- [ ] **Calendar/arrow day-change skeleton** — Use day arrows or the calendar to change days while frames are already on screen. Verify a segmented skeleton covers the strip, the frame strip stays mounted, playhead chip is hidden during the swap, and new-day frames land centered when the skeleton clears.
- [ ] **Strip skeleton icon chips** — Day-change and edge-prefetch skeletons use opacity-pulse pills with blank ~14px icon chips cycling soft / squircle / hard square.

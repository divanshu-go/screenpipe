...
- [ ] **Calendar jump to historical day** — Pick a day weeks/months back from the calendar. Verify the timeline actually switches to that day (no snap-back to today), even when the fetch takes 30s+; the date pill shows a spinner while a full-width segmented strip skeleton overlays the strip (opacity pulse — NOT opacity-50 dimming of the previous day; strip stays mounted underneath, playhead chip hidden), then the day swaps in atomically with the skeleton clearing.
...
- [ ] **Today button mid-swap** — Start a past-day navigation, then hit Today before it completes. Verify the swap cancels and today loads without a stuck date-swap skeleton overlay.
- [ ] **Calendar/arrow day-change skeleton (no remount)** — Use day arrows or the calendar icon to change days while frames are already on screen. Verify a full-width segmented skeleton covers the strip (opacity pulse, not opacity-50 previous-day bars), the frame strip DOM stays mounted (no half-strip / scroll jump from remounting), playhead chip is hidden during the swap, and new-day frames land cleanly centered when the skeleton clears.
- [ ] **Strip skeleton opacity pulse** — Day-change and edge-prefetch skeletons use segmented pills with opacity pulse and blank ~14px icon chips cycling soft / squircle / hard square by index (no random, no circles, glyphs, letters, Lucide brands, or reused yesterday icons; no backdrop-filter).
- [ ] **Post-swap playhead centering** — After a successful calendar/arrow day change, verify the strip centers on the landing frame, fills edge-to-edge wherever frame pills exist (no void between strip and screen edge mid-day), the playhead chip is visible, and the strip is the target day.
...
- [ ] **Edge prefetch skeleton** — Scroll the strip toward the end of the loaded day (either direction). Prefetch should arm ~2 viewports before the loaded frame-array edge (segmented opacity-pulse pills continue the strip — same as the calendar date-swap skeleton; no black void / scroll jerk), then swap to real bars when the batch lands. Crossing the day boundary with adjacent frames already merged should feel continuous — no layout jump when the virtualization window grows, and no void flash between skeleton and real bars. While viewing today, after an older-day edge prefetch completes, new live frames should keep arriving (live stream re-armed).

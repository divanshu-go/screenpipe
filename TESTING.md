...
- [ ] **Calendar jump to historical day (cold)** — Pick a day weeks/months back that is not already in the strip. Verify `pendingDateSwap` + full-width segmented opacity-pulse skeleton overlays the strip (frame row stays mounted underneath, playhead chip hidden); after `batch_complete` the day swaps in and the skeleton clears. Date stays on the picked day even if the fetch takes 30s+.
- [ ] **Calendar / arrow jump (hot)** — With a past day already in the in-memory strip *and* at least one older calendar day buffered (e.g. after scrolling/edge prefetch), pick that day from the calendar or arrows. Verify instant seek via `canInstantDateNav` — no skeleton, no refetch. A day present only as the prior sliver of a later cold fetch (e.g. Jul 7 after landing on Jul 8) must cold-fetch prior context instead of void-left at startOfDay.
- [ ] **Calendar pick during live streaming** — While today's frames are actively streaming, pick a past day. Verify only the picked day's swap is shown (no live-today frames mixed in) and the picked day loads.
...
- [ ] **Failed batch_complete unsticks swap** — If the server returns `batch_complete` with `error:true`, verify the date-swap skeleton clears promptly (not after 150s) and a load-failed toast appears with date restored.
- [ ] **Reconnect mid date-swap** — Kill/restart the engine while a past-day swap is pending. Verify reconnect re-fetches with prior-day context and the strip lands with content on both sides of the playhead.
- [ ] **Today button mid-swap** — Start a past-day navigation, then hit Today before it completes. Verify the swap cancels and today loads without a stuck date-swap skeleton overlay. Late batches from the cancelled past-day fetch must not merge into the strip (`suppressedNavRequestIds`), including across websocket reconnect mid-swap.
- [ ] **Hot jump cancels cold swap** — Start a cold jump to day A, then (once some other day B is already in the strip) hot-jump to B before A's batch_complete. Verify no skeleton, instant seek to B, and A's late frames do not appear in the strip.
- [ ] **Timeline section switch mid-swap** — Start cold calendar jump, switch home section away and back before batch_complete. Swap should complete or fail cleanly, not abort silently.
- [ ] **Prefetch during cold-nav await** — Start cold jump (slow prior-day SQL), scroll to strip edge before skeleton appears. Nav fetch must win; no stale prefetch frames after landing.
- [ ] **Nav fetch skips empty prior days** — Jump to a day whose previous calendar day is empty (e.g. Jul 1 after empty Jun 30). Verify arrival fetch starts at the nearest prior day with frames (Jun 29), so the strip lands with content on both sides of the playhead.
- [ ] **Long gap prior context (arrow / calendar)** — Arrow or cold-land onto a day after a recording gap longer than 7 empty days (e.g. Jun 1 then nothing until Jun 15). Verify `resolveNavFetchRange` still walks up to 365 days for prior context so the strip has content left of the playhead, not a thin target-only window.
- [ ] **Hot path after scrolled-in month** — After scrolling/edge-prefetching many past days into the strip (no more edge-prefetch skeletons), pick a mid-strip day via calendar or arrows. Verify instant hot seek (no date-swap skeleton). The oldest loaded day in the strip still cold-fetches for prior context (no void-left).
- [ ] **Wheel scrub + edge prefetch width** — While trackpad-scrubbing near a loaded edge, let adjacent-day skeleton/frames land mid-wheel. Verify the playhead stays on the same screen X (no one-frame hitch); idle at edge still preserves viewport X on merge.
- [ ] **Same-day calendar re-pick** — Already on a day with frames loaded; pick that day again from the calendar. Verify the playhead lands near the day's start via `findLoadedDayLandingIndex` (oldest end), not the newest same-day frame.
- [ ] **Search jump hot path** — With a past day already in the strip and its exact search `frame_id` loaded, click that search result. Verify instant seek (no date-swap skeleton) when the exact frame is present.
- [ ] **Strip skeleton icon chips** — Day-change and edge-prefetch skeletons use solid opacity-pulse pills with blank ~14px icon chips cycling soft / squircle / sharp (no fake app glyphs).
- [ ] **Post-swap playhead centering** — After a successful cold calendar/arrow day change, verify the strip centers on the landing frame, fills edge-to-edge wherever frame pills exist (no void between strip and screen edge mid-day), the playhead chip is visible, and the strip is the target day.
- [ ] **Mid-scroll strip edge-to-edge** — Scrub or wheel-navigate to mid-day (frames continue past both edges). Verify each viewport edge is flush-filled by the continuing bar (square/cut-off by the screen edge, not a black void + floating rounded pill start). True day start/end may still show `px-[50vw]` padding and rounded caps; small `segGap` between different app groups is intentional.
- [ ] **Audio-only day navigation** — On a day with only mic audio (screen recording off), verify the day is reachable from the calendar and arrows, the swap succeeds (no empty-day toast), the playhead lands on an audio marker, and transcriptions show in the subtitle bar.
...
- [ ] **Earliest recorded day reachable** — Navigate to the very first day that has any recording. Verify the calendar allows it and the day loads.
- [ ] **Edge prefetch skeleton** — Scroll toward the end of the loaded day (either direction). Prefetch should arm ~2 viewports before the loaded frame-array edge while moving toward it (segmented skeleton continues the strip); absolute ends (index 0 / last) may arm even with delta 0. Fast fling into the void at either absolute end should arm without needing a reverse flick first (clamped overscroll bumps prefetch; throttle stamps only after a successful arm). Skip empty gap days; still probe a gap day whose midnight was only covered by a prior multi-day nav range. Skeleton clears atomically with the frame flush (no scroll bump). Historical forward prepend keeps the playhead put (`followLiveEdge` only while viewing today). On today, after an older-day edge prefetch completes, new live frames should keep arriving.
- [ ] **Timeline survives Next HMR** — With `bun tauri dev` on today, scrub mid-strip, then save a timeline file (e.g. `timeline-live-edge.ts`) to trigger Fast Refresh. Verify the strip stays populated/scrubbable (not a sparse empty virtualized window), playhead still moves, and live frames keep arriving at index 0 when parked on the live edge. Mid-swap HMR must rehydrate `seekingTimestamp` from `pendingNavTargetDate` (swap continues); only true orphans (no target / not loading) clear `pendingDateSwap`.
- [ ] **Cold nav gates before prior-day SQL** — Start a cold calendar jump; confirm the date-swap skeleton appears before `resolveNavFetchRange` returns (no edge-prefetch / live merge window while prior-day lookup is in flight).

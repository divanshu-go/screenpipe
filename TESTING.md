...
- [ ] **Hot path needs near-midnight coverage** — With prior-day context buffered but only afternoon samples for the target day (sparse downsample), pick that day. Verify cold skeleton refetch (not instant hot), and landing includes morning/near-midnight frames when available.

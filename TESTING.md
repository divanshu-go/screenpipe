...
- [ ] **Failed batch_complete unsticks swap** — If the server returns `batch_complete` with `error:true`, verify the strip undims promptly (not after 150s) and a load-failed toast appears with date restored.
...
- [ ] **Today button mid-swap** — Start a past-day navigation, then hit Today before it completes. Verify the swap cancels and today loads without a stuck dimmed strip.

...
- [ ] **Today button mid-swap** — Start a past-day navigation, then hit Today before it completes. Verify the swap cancels and today loads without a stuck date-swap skeleton overlay. Late batches from the cancelled past-day fetch must not merge into the strip (`suppressedNavRequestIds`).

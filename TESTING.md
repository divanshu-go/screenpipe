...
- [ ] **Post-swap playhead centering** — After a successful calendar/arrow day change, verify the strip centers on the landing frame (not a half strip requiring a manual drag), the playhead chip is visible on that frame, and the strip is the target day (not index-0 end-of-day / prior-evening context).

...
- [ ] **Calendar / arrow jump (hot)** — With a past day already in the in-memory strip *and* at least one older calendar day buffered (e.g. after scrolling/edge prefetch), pick that day from the calendar or arrows. Verify instant seek via `canInstantDateNav` — no skeleton, no refetch — even when the earliest same-day frame is mid-morning (e.g. 8–9am). A day present only as the prior sliver of a later cold fetch (e.g. Jul 7 after landing on Jul 8) must cold-fetch prior context instead of void-left at startOfDay.
...
- [ ] **Hot path after scrolled-in month** — After scrolling/edge-prefetching many past days into the strip (no more edge-prefetch skeletons), pick a mid-strip day via calendar or arrows whose earliest frame is mid-morning or afternoon. Verify instant hot seek (no date-swap skeleton). The oldest loaded day in the strip still cold-fetches for prior context (no void-left).
...
- [ ] **Edge prefetch skeleton** — Scroll toward the end of the loaded day (either direction). Prefetch should arm ~2 viewports before the loaded frame-array edge while moving toward it (segmented skeleton continues the strip); absolute ends (index 0 / last) may arm even with delta 0. Skip empty gap days; still probe a gap day whose midnight was only covered by a prior multi-day nav range. Skeleton clears atomically with the frame flush (no scroll bump). Historical forward prepend keeps the playhead put (`followLiveEdge` only while viewing today). On today, after an older-day edge prefetch completes, new live frames should keep arriving.

...
- [ ] **Calendar / arrow jump (hot)** — With a past day already in the in-memory strip (e.g. after scrolling/edge prefetch), pick that day from the calendar or arrows. Verify instant seek via `canInstantDateNav` — no skeleton, no refetch.

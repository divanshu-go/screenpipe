...
- [ ] **Calendar jump to historical day** — Pick a day weeks/months back from the calendar. Verify the timeline actually switches to that day (no snap-back to today), even when the fetch takes 30s+; the date pill shows a spinner while a full-width strip skeleton overlays the strip (same shimmer language as edge prefetch — NOT opacity-50 dimming of the previous day; strip stays mounted underneath, playhead chip hidden), then the day swaps in atomically with the skeleton clearing.
...
- [ ] **Calendar/arrow day-change skeleton (no remount)** — Use day arrows or the calendar icon to change days while frames are already on screen. Verify a full-width shimmer skeleton covers the strip (not opacity-50 previous-day bars), the frame strip DOM stays mounted (no half-strip / scroll jump from remounting), playhead chip is hidden during the swap, and new-day frames land cleanly centered when the skeleton clears.
...
- [ ] **Edge prefetch shimmer** — Scroll the strip toward the end of the loaded day (either direction). Prefetch should arm ~2 viewports before the loaded frame-array edge (muted shimmer pills continue the strip — same pill language as the calendar date-swap skeleton; no black void / scroll jerk), then swap to real bars when the batch lands. Crossing the day boundary with adjacent frames already merged should feel continuous — no layout jump when the virtualization window grows, and no void flash between shimmer and real bars. While viewing today, after an older-day edge prefetch completes, new live frames should keep arriving (live stream re-armed).
